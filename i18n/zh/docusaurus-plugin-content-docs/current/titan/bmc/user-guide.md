...
Titan BMC 是一个用于管理 Milk-V Cluster 集群节点的 Web 管理工具。它提供了以下主要功能：
...
UART Agent 支持两种终端连接方式：

1. **UART**：通过串口连接，适用于系统启动、调试等场景
2. **TTY**：通过 SSH 伪终端连接，需要节点上安装 Agent
...
4. **输入节点凭据**
   ...
   在弹出的对话框中输入节点的登录信息：
   - **Username**：节点的 SSH 用户名（例如：`ubuntu`）
   - **Password**：节点的 SSH 密码
