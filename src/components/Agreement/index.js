...
export default (props) => {
    ...
    const zh = <>
        <div className={styles.cBox}>
            <h1>CV1800B手册保密条款</h1>
            <p>本手册由 北京晶视智能科技有限公司 制作，并包含关于CV1800B芯片的机密信息。为了保护这些机密信息，以下条款适用于手册的使用者（以下简称"使用者"）。</p>
            <h2>1. 保密义务</h2>
    ...
    const en = <>
        ...
        <div className={styles.marginBox}>
            <input type="radio" className={styles.square} required checked={agree} onChange={() => { }} onClick={() => setAgreeFlag(s => !s)} />
            <span>I agree to and will abide by the terms above</span>
        </div>
    </>
    return (
        <>
            {/* */}
            <div className={styles.agreement} style={{ display: `${dis ? "flex" : 'none'}` }} onClick={(event) => {
