...
The Titan motherboard has two DDR4 slots, the current firmware only supports DDR4 UDIMMs. Both memory slots must be populated, and the two memory modules installed must have identical specifications. For a list of currently verified DDR4 modules, please refer to the following link:
